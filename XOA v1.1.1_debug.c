...
while (1)                    //loop time = 4.5ms
	{ 		
	//	  AT24CXX_WriteOneByte(0x00, 0x38);
	 ...
	 if ((I2C0SSTA & 0x04)==0x04)   // Slave Transmit IRQ
	{
		...
		if(I2C0SRX == 0x00)          //1. Send SN:7b01 (7:2017 / b:November / 01:serial number)
		  {		
			...
			{		
			  I2C0STX = ((avg0 & 0xFF00) >> 8);   //Power_o
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
          }  
				I2C0STX = avg0;                    //Power_o
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
			...
			{		
			  I2C0STX = ((avg1 & 0xFF00) >> 8);   //Power_i
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
          }  
				I2C0STX = avg1;                      //Power_i
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
...
//memset(pp,0,sizeof(PID));    //failed ?
	...
	pp->Proportion = 0.01;    // Proportional Const   threshold = 2.0
	pp->Integral   = 0.01;      // Integral Const       
