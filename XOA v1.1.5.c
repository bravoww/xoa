...
#define  I_MAX           500     //max current(mA)
#define  P_MAX           200     //max Power * 10(dbm)
...
/*********	Make sure PIN_DEBUG is ready when TURN IT ON!!!  **********/
/*********	Make sure PIN_DEBUG is ready when TURN IT ON!!!  **********/
/*********	Make sure PIN_DEBUG is ready when TURN IT ON!!!  **********/	
...
/*****************************************************/	
		 ADCCP   = 0x04;				// conversion on ADC3   (SN:7B01   here should be ADC3)
		 sum = 0;
		 for(i=0;i<8;i++)
		 {			 
			 delay(1000);
...
/*****************************************************/			
		 T3CLRI = 0xAA;      //feed dog	 
		 delay(1000);
		 ADCCP   = 0x03;				// conversion on ADC4   (SN:7B01   here should be ADC4)
		 sum = 0;
		 for(i=0;i<8;i++)
		 {
			 delay(1000);
...
u8 IsValid_ACC(u16 x)
{
	double y;
	y =  x & 0x01FF; 
	if(y<=I_MAX)               //current = (0mA,200mA)	MAX = 450mA
	 return 1;
	else
	 return 0;
}
