...
//v1.1.6
//1.add macro (I_MAX/P_MAX/P_MIN/I_MAX_Debug/P_MAX_Debug/P_MIN_Debug)
...
#define  I_MAX           480     //max current(mA)
#define  P_MAX           170     //max Power * 10(dbm)
#define  P_MIN           0       //min Power * 10(dbm)
#define  I_MAX_Debug     480     //max current(mA)
#define  P_MAX_Debug     170     //max Power * 10(dbm)
#define  P_MIN_Debug     0       //min Power * 10(dbm)
...
double COE_i = 3.788;          //total input  coe  same direction   7C01:3.788  7C02:2.48    7C03:4.58
double COE_o = 0.0472;        //total output coe  same direction   7C01:0.0472 7C02:0.057   7C03:0.0758
u16   Bias_V = 0x480;
...
/*********	Make sure PIN_DEBUG is ready before TURN IT ON!!!  **********/
/*********	Make sure PIN_DEBUG is ready before TURN IT ON!!!  **********/	
if((GP1DAT&0x00000080) == 0x0)  // enter debug mode
	{
...
/*****************************************************/	
		 ADCCP   = 0x03;				// conversion on ADC3   (SN:7B01 should be ADC3)
		 sum = 0;
		 for(i=0;i<8;i++)
		 {			 
			 delay(1000);
...
/*****************************************************/			
		 T3CLRI = 0xAA;      //feed dog	 
		 delay(1000);
		 ADCCP   = 0x04;				// conversion on ADC4   (SN:7B01  should be ADC4)
		 sum = 0;
		 for(i=0;i<8;i++)
		 {
			 delay(1000);
...
{ 
	...
	if((x&0x8000) == 0x8000)
		...
		y =   x & 0x00FF; 
	if((y>=Pmin) && (y<=Pmax))     //power = (0dbm,10dbm)
	{
		y = dbm2mw(y/10);
	  z = COE_o * y;	  
	  y = (z*4096)/2.5;	
...
u8 IsValid_ACC(u16 x)
{
	double y;
	y =  x & 0x01FF; 
	if(y<=Imax)               //current = (0mA,200mA)	should never exceed 450mA
	 return 1;
	else
	 return 0;
}
...
void My_IRQ_Function()
{
  ...
  // Slave Recieve
	...
	// Slave Transmit
	 ...
	 {
		indx = 0;                    //clear?
		if(I2C0SRX == 0x00)          //1. Send SN:7b02 (7:2017 / b:November / 01:serial number)
		  {		
			  I2C0STX = 0x7c;
				k = 100;
