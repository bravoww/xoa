...
//v1.1.4
//1.add Bias_V(rDAC = rOUT + Bias_V)
//2.modify pid parameters
//v1.1.5
//1.add ACC;
//2.verfiy ACC & APC
//3.add IIC read:0x81+0x82+0x84
//v1.1.6
//1.add macro (I_MAX/P_MAX/P_MIN/Imax_Debug/Pmax_Debug/Pmin_Debug)
//2.delete chip temperature & NTC sample subprogram
//3.add cnt
//v1.1.7
//1.move down APC/ACC verify code(power up)
//v1.2.0
//1.add self calcuate coe_i&coe_o and save to FLASH
//2.debug & normal mode APC/ACC save to FLASH seperately
***************************************************************************/
...
#define  I_MAX           320     //max current(mA)
#define  P_MAX           150     //max Power * 10(dbm)
#define  P_MIN           0       //min Power * 10(dbm)
#define  Imax_Debug      450     //max current(mA)
#define  Pmax_Debug      170     //max Power * 10(dbm)
#define  Pmin_Debug      0       //min Power * 10(dbm)
#define  P_ini           130     //Power(default)  = 13.0dbm
#define  I_ini           320     //Current(default)= 320mA
#define  HARDWARE        0x11    //hardware version: v1.1
#define  SOFTWARE        0x20    //software version: v1.2.0
...
u16 APC,ACC,Power_i,Power_o,Current;       //  APC:target APC Power
u32 sum;
...
u16 GetTemp(u16 x);
u8 IsValid_APC(u16 x);
u8 IsValid_ACC(u16 x);
//---------------IIC functions-------------//
void IIC_Start(void);
void IIC_Stop(void);
...
u16 SelfCal_input(u16 x);
u16 SelfCal_output(u16 x);
int main()
{
	u8  i;
...
//GP4SET  = 0x00040000;    //P4.2=1(Small feedback)	
	...
	IRQ = My_IRQ_Function;
	IRQEN = SM_SLAVE_BIT;				//  I2C0 Slave Interupt (0x200)
...
/*********	Make sure PIN_DEBUG is ready before TURN IT ON!!!  **********/
/*********	Make sure PIN_DEBUG is ready before TURN IT ON!!!  **********/
/*********	Make sure PIN_DEBUG is ready before TURN IT ON!!!  **********/	
if((GP1DAT&0x00000080) == 0x0)  status = 0x83;    // debug  + ACC   
	else status = 0x43;                             // normal + APC 
if((status & 0x40) == 0x0)      //debug mode
	{
    ...
    T3CON  = 0xE0;    	  		  // IRQ instead of reset, can NOT be 0xF0	
		...
		Pmin = P_MIN;
		Imax = I_MAX;	
		tmp  = AT24CXX_ReadOneByte(0x01);
		tmp  = tmp << 8;
		tmp |= AT24CXX_ReadOneByte(0x00); 
		if(IsValid_APC(tmp))		APC = tmp;
		else		                APC = P_ini;          
		tmp  = AT24CXX_ReadOneByte(0x03);
		tmp  = tmp << 8;
		tmp |= AT24CXX_ReadOneByte(0x02);
		if(IsValid_ACC(tmp))		ACC = tmp;
		else	                  ACC = I_ini;          
	}	
	
		    	tmp = SelfCal_input(0xff);
					AT24CXX_WriteOneByte(0x04, tmp);			
					AT24CXX_WriteOneByte(0x05, tmp >> 8);
					tmp = SelfCal_output(0x88);
					AT24CXX_WriteOneByte(0x06, tmp);			
					AT24CXX_WriteOneByte(0x07, tmp >> 8);
	
		tmp  = AT24CXX_ReadOneByte(0x05);
		tmp  = tmp << 8;
		tmp |= AT24CXX_ReadOneByte(0x04); 
		coe_i= (double)tmp/1000;         //read coe_i from FLASH       
		tmp  = AT24CXX_ReadOneByte(0x07);
		tmp  = tmp << 8;
		tmp |= AT24CXX_ReadOneByte(0x06);
		coe_o= (double)tmp/1000;         //read coe_o from FLASH
/***********************************************************/
while(1)                    //loop time = 5.5ms
	{ 
...
/*****************************************************/	
		 ADCCP   = 0x04;				// conversion on ADC3   (SN:7B01 should be ADC3)
		 sum = 0;
		 for(i=0;i<8;i++)
		 {			 
			 delay(1000);
     ...
     if((status & 0x83) == 0x03)		    //PID works when LD enabled
		 {		
			rIn  = avg0;                     // Read Input 
			SetPdbm(APC,&sPID) ;
...
/*****************************************************/				 
   ...
   if((status & 0x83)==0x83)		    //PID works when LD enabled
		 {		
			rIn  = avg2;                 // Read Input   			 
			SetCurrent(ACC,&sPID);
			rOut = PIDCalc (&sPID,rIn ); // Perform PID Interation 
			rDAC = rOut + Bias_V;
		  SetDAC ( rDAC );             // Effect Needed Changes 
	 // SetDAC ( 0xD00); // Effect Needed Changes 			 
		 }
/*****************************************************/	
/************* executed every 0.5s  ******************/		
/*****************************************************/
 		if(cnt == 0x58)    //period = 0.5s
		{
			cnt = 0;
			GP0SET = 0x00800000;  
/*****************************************************/	
...
/*****************************************************/			
		 T3CLRI = 0xAA;      //feed dog	 
		 delay(1000);
		 ADCCP   = 0x03;				// conversion on ADC4   (SN:7B01  should be ADC4)
		 sum = 0;
		 for(i=0;i<8;i++)
		 {
			 delay(1000);
...
u16 SelfCal_input(u16 x)
{
	...
	u16 t;
	y = ((double)x/4096)*2.5;
	z = (1000*y)/0.031622776602;     //input = -15dbm
	t = (u16) z;
	return t;
}
u16 SelfCal_output(u16 x)
{
	...
	u16 t;
	y = ((double)x/4096)*2.5;
	z = (1000*y)/3.1622776602;     //output = 5dbm
	t = (u16) z;
	return t;
}
u16 Getcurrent(u16 x)
...
//3. 0.02/0.0032/0        option3(slow)
//4. 0.06/0.0096/0        sn:7B01
void PIDInit (PID *pp) 
{ 
//memset(pp,0,sizeof(PID));    //failed ?
	pp->SetPoint   = 330;     // Desired value  //327; 
	pp->Proportion = 0.04;    // Proportional Const   threshold = 2.0
	pp->Integral   = 0.0064;     // Integral Const    threshold = 8.0 
		  ...
		  if(data[0] == 0xFF) 
			{
				...
				{
					status &= 0x7F;         //APC_mode  
				}				
			}
			else if(data[0] == 0xFE)
			{
				data_tmp_16  = data[1] ;
				data_tmp_16  = data_tmp_16 << 8;
				data_tmp_16 |= data[2] ;  
        if((status & 0x80) == 0x80)
				{
					if(IsValid_ACC(data_tmp_16))		ACC = data_tmp_16;
				}		
				else
				{
					if(IsValid_APC(data_tmp_16))		APC = data_tmp_16;
				}
			}
			else if(data[0] == 0xFD)
			{
				if((status & 0x40) == 0x0)
				{
					data_tmp_16 = SelfCal_input(avg1);
					AT24CXX_WriteOneByte(0x04, data_tmp_16);			
					AT24CXX_WriteOneByte(0x05, data_tmp_16 >> 8);
					data_tmp_16 = SelfCal_output(avg0);
					AT24CXX_WriteOneByte(0x06, data_tmp_16);			
					AT24CXX_WriteOneByte(0x07, data_tmp_16 >> 8);
				}
				else
				{
					if((status & 0x80) == 0x80)
					{				
						AT24CXX_WriteOneByte(0x02, ACC);			
						AT24CXX_WriteOneByte(0x03, ACC >> 8);
					}
					else
					{
						AT24CXX_WriteOneByte(0x00, APC);		
						AT24CXX_WriteOneByte(0x01, APC >> 8);
					}//if((status & 0x80) == 0x80)
				}//if((status & 0x40) == 0x0)
			}//if(data[0] == 0xFD)
	}//if ((I2C0SSTA & 0x08)==0x08)
	// Slave Transmit
	if ((I2C0SSTA & 0x04)==0x04)   // Slave Transmit IRQ
	 {
		indx = 0;                    //clear?
		if(I2C0SRX == 0x00)          //1. Send version
		  {		
			  I2C0STX = SOFTWARE;          //1.1 Send software version
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
          }  				
				I2C0STX = HARDWARE;          //1.2 Send hardware version
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
			...
			{		
			  I2C0STX = ((Power_o & 0xFF00) >> 8);   //Power_o   
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
          }  
				I2C0STX = Power_o;                    //Power_o    
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
			...
			{		
			  I2C0STX = ((Power_i & 0xFF00) >> 8);   //Power_i     
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
          }  
				I2C0STX = Power_i;                      //Power_i    
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
			...
			{		
			  I2C0STX = ((Current & 0xFF00) >> 8);  //Current   avg2
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
          }  
				I2C0STX = Current;                     //Current   avg2
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
			...
			{		
			  I2C0STX = ((Temp & 0xFF00) >> 8);   //Temp  avg3   
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
          }  
				I2C0STX = Temp;                    //Temp  avg3
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
		 ...
		 else if(I2C0SRX == 0x81)    //7. Output Port AD
			{
				if((status & 0x40) == 0x0)
				{					
					I2C0STX = ((avg0 & 0xFF00) >> 8);   
					k = 100;
					while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
						{
							k--;
		...
		else if(I2C0SRX == 0x82)    //8.Input Port AD
			{	
        if((status & 0x40) == 0x0)
				{					
		...
		else if(I2C0SRX == 0x84)       //9.Sensor(Temperature)
			{	
			  if((status & 0x40) == 0x0)
				{					
