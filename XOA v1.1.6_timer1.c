...
//v1.1.6
//1.add macro (I_MAX/I_MIN/P_MAX/P_MIN)
...
double COE_o = 0.0732;        //total output coe  same direction   0.0472_7b01
u16   Bias_V = 0x480;
...
//GP4SET  = 0x00040000;    //P4.2=1(Small feedback)	
	...
	FIQEN = SM_SLAVE_BIT;	     	//	I2C0 Slave Interupt (0x200)
...
// I2C1DIV = 0x283C;				  // 0x283C = 400kHz  0xCFCF = 100kHz
  ...
  ADCPoweron(2000);		// Power on ADC		
	...
	T1LD = 0x4000000;					// Counter Value   period = 100ms   0x4000000 0x40000
...
//memset(pp,0,sizeof(PID));    //failed ?
	...
	pp->Proportion = 0.06;    // Proportional Const   threshold = 2.0
	pp->Integral   = 0.0096;     // Integral Const    threshold = 8.0 
...
u8 IsValid_APC(u16 x)
{
	...
	if((x&0x8000) == 0x8000)
		...
		y =   x & 0x00FF; 
	if((y>=P_MIN) && (y<=P_MAX))     //power = (0dbm,10dbm)	
	 return 1;
	else
	 return 0;
}
u8 IsValid_ACC(u16 x)
{
	double y;
	y =  x & 0x01FF; 
	if(y<=I_MAX)               //current = (0mA,200mA)	should never exceed 450mA
	 return 1;
	else
	 return 0;
}
