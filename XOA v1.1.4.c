...
/*****************************************************/	
	...
	//	SetDAC ( 0x485 ); // Effect Needed Changes 				 
		 }				 
/*****************************************************/	
	 ...
	 if ((I2C0SSTA & 0x04)==0x04)   // Slave Transmit IRQ
	{
		...
		if(I2C0SRX == 0x00)          //1. Send SN:7b01 (7:2017 / b:November / 01:serial number)
		  {		
			...
			{		
			  I2C0STX = ((Power_o & 0xFF00) >> 8);   //Power_o   avg0
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
          }  
				I2C0STX = Power_o;                    //Power_o    avg0
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
			...
			{		
			  I2C0STX = ((Power_i & 0xFF00) >> 8);   //Power_i     avg1
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
          }  
				I2C0STX = Power_i;                      //Power_i    avg1
				k = 100;
				while(((I2C0SSTA & 0x04) == 0x00) && (k > 0))  //Wait for end of sending
					{
            k--;
...
//3. 0.02/0.0032/0        option3
void PIDInit (PID *pp) 
{ 
//memset(pp,0,sizeof(PID));    //failed ?
	pp->SetPoint   = 330;     // Desired value  //327; 
	pp->Proportion = 0.02;    // Proportional Const   threshold = 2.0
	pp->Integral   = 0.0032;      // Integral Const    threshold = 8.0 
